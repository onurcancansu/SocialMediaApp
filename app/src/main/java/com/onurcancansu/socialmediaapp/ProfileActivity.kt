...
import com.onurcancansu.socialmediaapp.adapter.ProfileVideoAdapter
import com.onurcancansu.socialmediaapp.databinding.ActivityProfileBinding
    ...
    fun setUI(){
        profileUserModel.apply {
            Glide.with(binding.profilePic).load(profilePic)
                .apply(RequestOptions().placeholder(R.drawable.account_circle))
                .circleCrop()
    ...
    fun setupRecyclerView(){
        val options = FirestoreRecyclerOptions.Builder<VideoModel>()
            .setQuery(
                Firebase.firestore.collection("videos")
                    .whereEqualTo("uploaderid", profileUserId)
                    .orderBy("createdtime",Query.Direction.DESCENDING),VideoModel::class.java
            ).build()
